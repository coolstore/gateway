        ...
        rest("/cart/").description("Personal Shopping Cart Service")
            ...
            .post("/checkout/{cartId}").description("Finalize shopping cart and process payment")
                ...
                .hystrix().id("Cart Service (Checkout Cart)")
                    ...
                    .setHeader(Exchange.HTTP_URI, simple("http://{{env:CART_ENDPOINT:shoppingcart:8080}}/api/cart/checkout/${header.cartId}"))
                    .to("http4://DUMMY")
                .onFallback()
            ...
            .get("/{cartId}").description("Get the current user's shopping cart content")
                ...
                .hystrix().id("Cart Service (Get Cart)")
                    ...
                    .setHeader(Exchange.HTTP_URI, simple("http://{{env:CART_ENDPOINT:shoppingcart:8080}}/api/cart/${header.cartId}"))
                    .to("http4://DUMMY")
                .onFallback()
                    .setHeader(Exchange.HTTP_RESPONSE_CODE, constant(Response.Status.SERVICE_UNAVAILABLE.getStatusCode()))
        			.setHeader(Exchange.CONTENT_TYPE, constant("application/json"))
            ...
            .delete("/{cartId}/{itemId}/{quantity}").description("Delete items from current user's shopping cart")
                ...
                .hystrix().id("Cart Service (Delete Cart)")
                    ...
                    .setHeader(Exchange.HTTP_URI, simple("http://{{env:CART_ENDPOINT:shoppingcart:8080}}/api/cart/${header.cartId}/${header.itemId}/${header.quantity}"))
                    .to("http4://DUMMY")
                .onFallback()
                    .setHeader(Exchange.HTTP_RESPONSE_CODE, constant(Response.Status.SERVICE_UNAVAILABLE.getStatusCode()))
        			.setHeader(Exchange.CONTENT_TYPE, constant("application/json"))
            ...
            .post("/{cartId}/{tmpId}").description("Transfer temp shopping items to user's cart")
                ...
                .setBody(simple("null"))
                .setHeader(Exchange.HTTP_METHOD, HttpMethods.POST)
                .setHeader(Exchange.HTTP_URI, simple("http://{{env:CART_ENDPOINT:shoppingcart:8080}}/api/cart/${header.cartId}/${header.tmpId}"))
                .to("http4://DUMMY")
                .onFallback()
                    .setHeader(Exchange.HTTP_RESPONSE_CODE, constant(Response.Status.SERVICE_UNAVAILABLE.getStatusCode()))
        			.setHeader(Exchange.CONTENT_TYPE, constant("application/json"))
